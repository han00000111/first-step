...
export function TaskEntryForm() {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 text-sm font-medium text-[#62786d]">
      ...
      <textarea
        name="content"
        required
        className="mt-3 min-h-40 w-full resize-none rounded-[26px] border border-[#d9e3dc] bg-[#fcfdfb] px-5 py-5 text-base leading-7 text-zinc-900 shadow-[inset_0_1px_0_rgba(255,255,255,0.85)] transition focus:border-[#c5d2ca] focus:bg-white focus:shadow-[0_0_0_4px_rgba(197,210,202,0.35)] lg:min-h-[18rem] lg:px-6 lg:py-6 lg:text-[1.05rem] lg:leading-8 xl:min-h-[20rem]"
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        {quickExamples.map((example) => (
          <span
            key={example}
            className="inline-flex items-center gap-2 rounded-full border border-[#d9e3dc] bg-[#f1f5f2] px-3 py-1.5 text-xs text-[#5d7469]"
          >
            <Sparkles className="h-3 w-3 text-[#62786d]" />
            {example}
          </span>
        ))}
      </div>

      <details className="group mt-5 rounded-[24px] border border-[#dce4de] bg-[#f6f8f5] p-4">
        ...
        <div className="mt-4 grid gap-4">
          <div>
            ...
            <input
              name="dueAt"
              type="datetime-local"
              className="mt-2 w-full rounded-2xl border border-zinc-200 bg-white px-4 py-3 text-sm text-zinc-800 transition focus:border-[#c5d2ca] focus:bg-white focus:shadow-[0_0_0_4px_rgba(197,210,202,0.28)]"
            />
                  ...
                  <button
                    ...
                    className={cn(
                      ...
                      preferredContext === option.value
                        ? "border-[#c8d5ce] bg-[#edf3ef] text-[#566d62]"
                        : "border-zinc-200 bg-white text-zinc-600 hover:border-[#d9e3dc] hover:bg-[#f1f5f2]",
                    )}
                  >
      ...
      <div className="mt-5 flex flex-col gap-3">
        <div className="text-sm text-zinc-500">
          {state.message ? (
            <span
              className={cn(
                state.status === "error" ? "text-[#8a6c6c]" : "text-[#62786d]",
              )}
            >
              {state.message}
            </span>
        ...
        <FormSubmitButton
          pendingText="加入中..."
          className="w-full bg-[#60796e] px-5 py-3.5 text-base text-white shadow-[0_14px_30px_-18px_rgba(96,121,110,0.52)] hover:bg-[#556d63]"
        >
          加入任务
        </FormSubmitButton>
      </div>
